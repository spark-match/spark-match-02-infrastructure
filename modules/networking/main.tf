...
resource "aws_vpc" "main" {
  ...
  #
  # Habia aqui un `lifecycle { prevent_destroy = true }` sobre la VPC.
...
# AWS crea automaticamente un SG "default" en cada VPC que permite la
# totalidad del trafico intra-SG (ingress 0.0.0.0/0 al mismo SG). Es un
# riesgo de compliance (CIS AWS Foundations Benchmark control 5.3). Al
# reescribirlo con ingress=[] y egress=[], Terraform lo deja vacio de forma
# efectiva: cualquier instancia/recurso que termine asociada al SG default
# no tendra reglas de trafico. Esto obliga a usar SGs explicitos.
